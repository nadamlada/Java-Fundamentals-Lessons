...
public class X10PokeMon {
    public static void main(String[] args) {
        ...
        int nAllPower = Integer.parseInt(scanner.nextLine());
        int m = Integer.parseInt(scanner.nextLine());
        int y = Integer.parseInt(scanner.nextLine());
        int counterPoked = 0;

        double halfValue = nAllPower * 0.5;

        while (nAllPower >= m) {
            nAllPower -= m;
            counterPoked++;

            if (nAllPower == halfValue) {            //едното е инт, едното е дъбъл, работи
                if (y != 0.0)                        //само тогава може да се дели, когато делителя не е нула
