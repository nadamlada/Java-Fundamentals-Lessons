...
public class EX01CounterStrike {
    public static void main(String[] args) {
        ...
        int allPower = Integer.parseInt(scanner.nextLine());
        ...
        while (!distance.equals("End of battle")) {
            int distanceParced = Integer.parseInt(distance);

            if (allPower < distanceParced) {
                wonBattle = false;
                break;
            } else {
                counterWon++;
                allPower -= distanceParced;
                wonBattle = true;
        ...
        if (wonBattle) {
            System.out.printf("Won battles: %s. Energy left: %s", counterWon, allPower);
        } else {
            System.out.printf("Not enough energy! Game ends with %s won battles and %s energy", counterWon, allPower);
        }
    }
}